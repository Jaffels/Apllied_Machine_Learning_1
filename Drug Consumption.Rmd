...
```{r pois9, echo=FALSE, cache=TRUE, fig.width=7, fig.height=5}

if (exists("cannabis_model") && exists("model_data") && exists("predictors")) {
  
  model_coeffs <- coef(cannabis_model)
  
  # --- Calculate effect for Age ---
  age_range <- 1:6 
  
  # Hold other predictors at their means for Age effect
  mean_values_for_age_effect <- colMeans(model_data[, setdiff(predictors, "Age")], na.rm = TRUE)
  
  age_linear_predictor_base <- model_coeffs["(Intercept)"]
  for (pred_name in names(mean_values_for_age_effect)) {
    if (pred_name %in% names(model_coeffs) && !is.na(mean_values_for_age_effect[pred_name])) {
      age_linear_predictor_base <- age_linear_predictor_base + 
        model_coeffs[pred_name] * mean_values_for_age_effect[pred_name]
    }
  }
  # Ensure 'Age' coefficient exists before using it
  if ("Age" %in% names(model_coeffs)) {
    age_predicted_values <- exp(age_linear_predictor_base + model_coeffs["Age"] * age_range)
  } else {
    # Fallback if 'Age' coefficient is missing (should not happen if model is correct)
    age_predicted_values <- rep(NA, length(age_range)) 
    warning("Coefficient for 'Age' not found in cannabis_model for pois9 plot.")
  }
  
  age_effect_df <- data.frame(
    Value = age_range,
    Predicted = age_predicted_values,
    Predictor_Type = "Age Category (1=18-24 to 6=65+)",
    stringsAsFactors = FALSE
  )
  age_effect_df <- age_effect_df[!is.na(age_effect_df$Predicted), ]

  # --- Calculate effect for Sensation Seeking (SS) ---
  ss_range <- seq(min(model_data$SS, na.rm = TRUE), max(model_data$SS, na.rm = TRUE), length.out = 100)
  
  # Hold other predictors at their means for SS effect
  mean_values_for_ss_effect <- colMeans(model_data[, setdiff(predictors, "SS")], na.rm = TRUE)
  
  ss_linear_predictor_base <- model_coeffs["(Intercept)"]
  for (pred_name in names(mean_values_for_ss_effect)) {
    if (pred_name %in% names(model_coeffs) && !is.na(mean_values_for_ss_effect[pred_name])) {
      ss_linear_predictor_base <- ss_linear_predictor_base + 
        model_coeffs[pred_name] * mean_values_for_ss_effect[pred_name]
    }
  }
  # Ensure 'SS' coefficient exists
  if ("SS" %in% names(model_coeffs)) {
    ss_predicted_values <- exp(ss_linear_predictor_base + model_coeffs["SS"] * ss_range)
  } else {
    ss_predicted_values <- rep(NA, length(ss_range))
    warning("Coefficient for 'SS' not found in cannabis_model for pois9 plot.")
  }
  
  ss_effect_df <- data.frame(
    Value = ss_range,
    Predicted = ss_predicted_values,
    Predictor_Type = "Sensation Seeking Score",
    stringsAsFactors = FALSE
  )
  ss_effect_df <- ss_effect_df[!is.na(ss_effect_df$Predicted), ]

  # --- Combine data for plotting ---
  combined_plot_data <- rbind(age_effect_df, ss_effect_df)

  # --- Create faceted plot ---
  if (nrow(combined_plot_data) > 0) {
    faceted_plot <- ggplot(combined_plot_data, aes(x = Value, y = Predicted)) +
      geom_line(aes(color = Predictor_Type), linewidth = 1) +
      # Add points only for the 'Age Category' facet
      geom_point(data = subset(combined_plot_data, Predictor_Type == "Age Category (1=18-24 to 6=65+)"), 
                 aes(color = Predictor_Type), size = 3) +
      # ncol = 2 for side-by-side plots
      facet_wrap(~ Predictor_Type, scales = "free_x", ncol = 2) + 
      scale_color_manual(values = c("Age Category (1=18-24 to 6=65+)" = "blue", 
                                    "Sensation Seeking Score" = "red")) +
      theme_minimal() +
      labs(title = "Effect of Age and Sensation Seeking on Predicted Cannabis Usage",
           x = "Predictor Value",
    ...
    cat("Not enough data to generate the faceted plot (e.g., due to missing coefficients or NA predicted values).\n")
  }
} else {
  cat("Required objects (cannabis_model, model_data, predictors) not found to generate plots in pois9.\n")
}
```
...
```{r pois13, echo=FALSE, cache=TRUE, fig.width=6, fig.height=4, results='asis'}
# Check for multicollinearity and format as a styled table
# Ensure the 'car' package is loaded for vif()
if (requireNamespace("car", quietly = TRUE) && exists("cannabis_model")) {
  
  # Check if the model has an intercept, vif function might behave differently or error
  # For GLMs, typically an intercept is present.
  # Also, ensure there are at least 2 predictors for VIF calculation.
  if (length(coef(cannabis_model)) > 1 && length(names(coef(cannabis_model))) - sum(grepl("Intercept", names(coef(cannabis_model)))) >=2 ) {
    
    vif_values <- tryCatch({
      car::vif(cannabis_model)
    }, error = function(e) {
      cat("Error in VIF calculation for pois13: ", e$message, "\n")
      NULL
    })

    if (!is.null(vif_values)) {
      # Convert VIF values to a data frame for better table formatting
      # VIF can be a single vector or a matrix if terms have multiple df
      if (is.matrix(vif_values)) {
        # Handle terms with multiple df (e.g., factors, polynomials) - sum VIFs or take generalized VIF (GVIF^(1/(2*Df)))
        # For simplicity, this example will use the first column if it's a matrix, 
        # or you might need to adapt based on GVIF if that's what car::vif returns for GLMs with factors.
        # Assuming vif_values are for predictors and not an issue of multicollinearity with intercept.
        vif_df <- data.frame(
          Variable = rownames(vif_values),
          VIF = vif_values[,1], # Taking the first column, adjust if GVIF^(1/(2*Df)) is needed
          stringsAsFactors = FALSE
        )
      } else { # It's a vector
        vif_df <- data.frame(
      ...
      vif_df$Variable <- sapply(vif_df$Variable, function(x) {
        if (x %in% names(var_display_names_vif)) {
          return(var_display_names_vif[x])
        }
        return(x)
      })
      ...
      vif_df$Concern_Level <- cut(vif_df$VIF, 
                              ...
                              include.lowest = TRUE, # ensure 0 is included if VIF is very low
                              right = FALSE)
      ...
      # Create a nicely formatted table with kable
      vif_table_output <- kable(vif_df,
            format = "latex", # Ensure LaTeX output for results='asis'
            caption = "Multicollinearity Assessment - Variance Inflation Factors", 
            booktabs = TRUE,
            col.names = c("Predictor", "VIF", "Concern Level"),
            digits = 2,
            align = c('l', 'r', 'c'),
            escape = FALSE) %>% # escape = FALSE for any special characters if they were in variable names
        kable_styling(latex_options = c("striped", "hold_position"), 
                      full_width = FALSE,
                      position = "center") %>%
        row_spec(0, bold = TRUE) %>%
        column_spec(1, bold = TRUE) 
        # Conditional coloring of Concern_Level column might require more complex latex_column_spec or cell_spec
        # For simplicity, direct coloring in kable is sometimes tricky for PDF.
        # The following is an attempt but might need latex_column_spec for robust PDF coloring:
        # column_spec(3, color = ifelse(vif_df$Concern_Level %in% c("High", "Very High"), "red",
        #                             ifelse(vif_df$Concern_Level == "Moderate", "orange", "blue")))
      # Footnote has been removed

      print(vif_table_output)
    } else {
      cat("VIF values could not be generated for the cannabis_model in pois13.\n")
    }
  } else {
    cat("Not enough predictors in cannabis_model to calculate VIF in pois13 (need at least 2 predictors excluding intercept).\n")
  }
} else {
  if (!exists("cannabis_model")) {
    cat("Object 'cannabis_model' not found. Cannot calculate VIF in pois13.\n")
  }
  if (!requireNamespace("car", quietly = TRUE)) {
    cat("Package 'car' is not installed. Cannot calculate VIF in pois13.\n")
  }
}
```
